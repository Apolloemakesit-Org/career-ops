...
export async function syncCloudRunner({
  ...
  const aiGateway = config.aiProxyApiKey
    ? await listModels({ apiKey: config.aiProxyApiKey, rootUrl }).catch(error => ({ ok: false, status: 0, ids: [], error: error.message }))
  ...
  if (claimed?.runner === 'test-ai') {
    await runAiTestCommand({ client, command: claimed, config, testModel, rootUrl });
  } else if (claimed?.runner === 'test-cheap-ai') {
    await runCheapAiTestCommand({ client, command: claimed, config, testCheapModels, rootUrl });
...
async function runAiTestCommand({ client, command, config, testModel, rootUrl }) {
...
async function runCheapAiTestCommand({ client, command, config, testCheapModels, rootUrl }) {
  const results = await testCheapModels({ apiKey: config.aiProxyApiKey, rootUrl });
  await client.updateRunnerCommand(command.id, {
